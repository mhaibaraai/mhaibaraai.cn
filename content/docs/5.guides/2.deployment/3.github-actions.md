...
sudo docker run -d \
  ...
  ghcr.io/mhaibaraai/movk-nuxt-docs:latest
```
...
**首次启动容器**：`docker run` 加入 `webnet` 网络，设置对应环境变量。
...
sudo docker pull ghcr.io/mhaibaraai/movk-nuxt-docs:latest
sudo docker stop movk-nuxt-docs && sudo docker rm movk-nuxt-docs
# 然后重新 docker run
```
